"""
Context Manager for Bugzy AMS Chatbot.
...
def build_profile_memory(state: State) -> str:
    """Extract ONLY essential demographics + critical health info + AMS questionnaire fields.

    Include: name, age, height, weight, BMI, health_conditions, allergies, medications
    And AMS-specific fields: diet_preference, cuisine_preference, current_dishes, etc.
    ...
    def format_hw(_, s):
        ...
        res = []
        if h: res.append(f"Height: {h}")
        if w: res.append(f"Weight: {w}")
        return ", ".join(res)
    ...
    mapping = [
        ...
        {"key": "sedentary_time", "label": "Sedentary Time"},
...
def build_system_memory(state: State) -> str:
    ...
    # AMS has both Meal (7-day) and Exercise (7-day) plans
    if state.get("meal_plan_sent"):
         parts.append("Meal plan: Active (7-Day AMS Meal Plan)")
    else:
         parts.append("Meal plan: Not created yet")
         
    if state.get("exercise_plan_sent"):
         parts.append("Exercise plan: Active (7-Day AMS Exercise Plan)")
    else:
         parts.append("Exercise plan: Not created yet")
...
def _get_meal_plan_content(state: State, max_chars: int) -> str:
    ...
    product = state.get("product", "ams")  # Default to AMS for this module
    meal_plan_db = {}
